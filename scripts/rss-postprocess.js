...
const inputDir = path.resolve(
  cwd,
  getArgValue("--input-dir", process.env.RSS_INPUT_DIR || ".tmp/rss-sync"),
);
const outputDir = path.resolve(cwd, getArgValue("--output-dir", process.env.RSS_OUTPUT_DIR || "."));
const feedUrl = getArgValue(
  "--feed-url",
  process.env.RSS_FEED_URL || "https://yournewsletter.substack.com/feed",
);
...
async function fetchFeedEntries(url) {
  if (!url || url === "https://yournewsletter.substack.com/feed") {
    console.warn("RSS_FEED_URL is not configured. Skipping post-processing.");
    return new Map();
  }

  const response = await fetch(url, {
    headers: {
      "user-agent": "hashnode-sync-rss/1.0",
...
function formatDateTime(dateValue, fallbackFile) {
...
function formatFrontMatter({ title, description, canonicalUrl, coverImage, tags, publishedAt }) {
  ...
  return [
    ...
    `subtitle: '${escapeValue(description)}'`,
    `coverImageUrl: ${coverImage || ""}`,
    `publishedAt: '${publishedAt}'`,
    `originalArticleURL: ${canonicalUrl || ""}`,
    `ogTitle: '${escapeValue(title)}'`,
    `ogDescription: '${escapeValue(description)}'`,
    `ogImage: ${coverImage || ""}`,
    "tags:",
...
async function main() {
  ...
  for (const filePath of stubFiles) {
    ...
    const link = String(
      parsed.attributes.originalArticleURL || parsed.attributes.canonical_url || "",
    ).trim();
    ...
    const targetPath = path.join(outputDir, `${publishedAt.slice(0, 10)}-${slug}.md`);
    ...
    const description = extractDescription(
      sourceHtml,
      String(parsed.attributes.subtitle || parsed.attributes.description || ""),
    );
    const coverImage =
      extractCoverImage(sourceHtml) ||
      String(parsed.attributes.coverImageUrl || parsed.attributes.cover_image || "").trim();
